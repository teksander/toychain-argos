...
class RGBLEDs(object):
    ...
    def setLED(self, RGB):
        if not self.frozen:
            for i in range(3):
                self.robot.epuck_leds.set_single_color(self.all[i], RGB[i])

    def flashRed(self, delay=1):
        if not self.frozen:
            self.setLED(3*[self.red])
            time.sleep(delay)
            self.setLED(3*[self.off])

    def flashGreen(self, delay=1):
        if not self.frozen:
            self.setLED(3*[self.green])
            time.sleep(delay)
            self.setLED(3*[self.off])

    def flashBlue(self, delay=1):
        if not self.frozen:
            self.setLED(3*[self.blue])
            time.sleep(delay)
            self.setLED(3*[self.off])

    def flashWhite(self, delay=1):
        if not self.frozen:
            self.setLED(3*[self.white])
            time.sleep(delay)
            self.setLED(3*[self.off])
